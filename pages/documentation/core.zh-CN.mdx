...
如果您在寻找诸如 `Player`、`GameProviders` 或 `Stage` 等组件，可以在 [Player](/documentation/player) 章节中找到它们。

## 核心

核心是 NarraLeaf-React 的主要功能。您可以使用其抽象类来创建故事。
它包含三个部分：

[元素](./core/elements) 是 NarraLeaf-React 的基本构建块，用于在故事中创建动作。  
[动画](./core/animation) 用于为元素创建复杂动画。  
[游戏](./core/game) 是表示游戏当前状态的主类。例如，您可以使用 [LiveGame](./core/game/live-game) 来保存和加载游戏。
...
在此示例中，我们有一个包含两个角色的场景。
第一个角色说「Hello, World!」和「Good morning!」，
第二个角色说「Hi, John!」和「How are you?」。

这两个角色是元素，场景是场景，动作是对话。
...
```ts
const character1 = new Character("角色 1");
...
<Tabs items={['src/story.ts', 'src/page.tsx']}>
    <Tabs.Tab>
        ...
        // 定义你的角色
        const character1 = new Character("角色 1");
        const character2 = new Character("角色 2");
...
<Cards num={1}>
    <Cards.Card
        title="元素"
        description="了解如何使用 NarraLeaf-React 创建更复杂的动作。"
